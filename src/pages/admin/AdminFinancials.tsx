              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border/50">
                    ...
                    <th className="text-right font-medium text-muted-foreground p-4 text-xs uppercase tracking-wider">Tax (VAT)</th>
                ...
                <tbody>
                  {orders.map((order) => (
                    <tr key={order.id} className="border-b border-border/30 last:border-0 hover:bg-muted/30 transition-colors">
                      ...
                      <td className="p-4 text-right tabular-nums">€{order.tax.toLocaleString()}</td>
                      <td className="p-4 text-right tabular-nums">€{(order.total - order.tax).toLocaleString()}</td>
