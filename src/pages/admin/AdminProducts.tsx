...
const AdminProducts = () => {
  ...
  const [form, setForm] = useState({ name: "", slug: "", price: "", stock: "", dimensions: "", weight: "", description: "", category: "", room: "", material: "", color: "", image_url: "" });
  ...
  const handleCreate = async () => {
    ...
    setAddOpen(false);
    setForm({ name: "", slug: "", price: "", stock: "", dimensions: "", weight: "", description: "", category: "", room: "", material: "", color: "", image_url: "" });
  };
                      ...
                      <tr key={product.id} className="border-b border-border/30 last:border-0 hover:bg-muted/30 transition-colors">
                        <td className="p-4">
                          <div className="flex items-center gap-3">
                            {product.image_url && <img src={product.image_url} alt={product.name} className="w-10 h-10 rounded-md object-cover" />}
                            <span className="font-medium">{product.name}</span>
                          </div>
                        </td>
                        <td className="p-4 text-muted-foreground">{product.category}</td>
                        <td className="p-4 text-right tabular-nums">€{Number(product.price).toLocaleString()}</td>
